...
const generatePageMap = (options: SitemapOptions, app: App): Map<string, SitemapPageInfo> => {
    const {
        changefreq = 'daily',
        exclude = [],
...
export const generateSiteMap = async (options: SitemapOptions, app: App): Promise<void> => {
    ...
    const outFile = options.outFile ? options.outFile.replace(/^\//u, '') : 'sitemap.xml';
    const {
        dir,
        options: { base },
    } = app;
    ...
    pagesMap.forEach((page, path) => {
        if (!exclude.includes(path)) sitemap.write({ url: `${base}${path.replace(/^\//u, '')}`, ...page });
    });

    urls.forEach((item) => sitemap.write(item));
    sitemap.end();

    console.log(blue('Sitemap:'), black.bgGreen('Success'), `Sitemap generated and saved to ${cyan(outFile)}`);

    const robotTxtPath = dir.dest('robots.txt');
    const robotsTxt = existsSync(robotTxtPath) ? await readFile(robotTxtPath, { encoding: 'utf8' }) : '';
    ...
    console.log(blue('Sitemap:'), black.bgGreen('Success'), `Appended sitemap path to ${cyan('robots.txt')}`);
};
