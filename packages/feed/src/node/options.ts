...
import type { FeedChannelOption, FeedLinks, FeedOptions, FeedOutput } from './types';
...
export const checkOptions = (options: Partial<FeedOptions>, app: App): FeedOptions | false => {
    const { themeConfig } = app.options;
    const hostname = options.hostname || (themeConfig.hostname as string | undefined);
...
export const getFeedChannelOption = (options: FeedOptions, app: App): FeedChannelOption => {
    const { rootLang, hostname, icon, image } = options;
    const { base, themeConfig } = app.options;
    const { title, description } = app.siteData;
    const author = options.channel?.author?.name || (themeConfig.author as string | undefined);

    const copyright: string =
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        (themeConfig?.footer?.copyright as string | undefined) || (author ? `Copyright by ${author}` : '');
...
export const getFeedLinks = (options: FeedOptions, output: ResolvedFeedOutput, app: App): FeedLinks => {
