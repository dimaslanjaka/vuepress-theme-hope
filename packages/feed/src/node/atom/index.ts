...
import type { AtomAuthor, AtomCategory, AtomContent, AtomEntry } from './typings';
...
export const renderAtom = (feed: Feed): string => {
    ...
    const content: AtomContent = {
        ...
        feed: {
            ...
            ...(channel.author ? { author: genAuthororContributor(channel.author) } : {}),

            updated: channel.lastUpdated ? channel.lastUpdated.toISOString() : new Date().toISOString(),
            generator: generator,
    ...
    content.feed.entry = feed.items.map((item) => {
        ...
        if (Array.isArray(item.author))
            entry.author = item.author.filter((author) => author.name).map((author) => genAuthororContributor(author));
        else if (item.author && item.author.name) entry.author = [genAuthororContributor(item.author)];
