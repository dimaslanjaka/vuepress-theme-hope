...
import type { RSSCategory, RSSContent, RSSEnclosure, RSSGuid, RSSItem } from './typings';
...
export const renderRSS = (feed: Feed): string => {
    ...
    const content: RSSContent = {
        ...
        rss: {
            ...
            channel: {
                ...
                pubDate: {
                    _text: channel.pubDate ? channel.pubDate.toUTCString() : new Date().toUTCString(),
                },
                lastBuildDate: {
                    _text: channel.lastUpdated ? channel.lastUpdated.toUTCString() : new Date().toUTCString(),
                },
    ...
    if (channel.copyright) content.rss.channel.copyright = { _text: channel.copyright };
    ...
    content.rss.channel.item = feed.items.map((entry) => {
        ...
        if (entry.description) item.description = { _text: encodeXML(entry.description) };
        ...
        if (Array.isArray(entry.category)) {
            item.category = entry.category.filter((category) => category.name).map((category) => genCategory(category));
    ...
    if (hasContent) {
        content.rss._attributes['xmlns:content'] = 'http://purl.org/rss/1.0/modules/content/';
