...
export const renderJSON = (feed: Feed): string => {
    ...
    content.items = feed.items.map((item) => {
        ...
        if (item.lastUpdated) feedItem.date_modified = item.lastUpdated.toISOString();

        // author
        if (Array.isArray(item.author)) {
            feedItem.authors = item.author.filter((author) => author.name).map((author) => formatAuthor(author));
        } else if (typeof item.author === 'object') feedItem.authors = [formatAuthor(item.author)];
