import { checkOptions, getFeedChannelOption, getFeedLinks, getOutput } from './options';
...
const isFeed = (frontmatter: PageFrontmatter<FeedPluginFrontmatter>): boolean =>
    !frontmatter.home && frontmatter.article !== false && (!frontmatter.feed || frontmatter.feed.enable !== false);
...
const feedPlugin: Plugin<FeedOptions> = (options, app) => {
    ...
    return {
        ...
        async generated(): Promise<void> {
            const feedPages = (typeof pluginOptions.sort === 'function' ? pages.sort(pluginOptions.sort) : pages).slice(
                0,
                pluginOptions.count
            );
