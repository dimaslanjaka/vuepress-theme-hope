import { getImageMineType, isAbsoluteUrl, isUrl, resolveHTML, resolveUrl } from './utils';

import { BaseThemeConfig, getAuthor, resolveCategory } from '@mr-hope/vuepress-shared';
...
export class FeedPage {
    ...
    constructor(private $page: Page, private feed: Feed, private options: FeedOptions, private app: App) {
    ...
    get author(): FeedAuthor[] {
        ...
        if (typeof this.feedOption.author === 'object') return [this.feedOption.author];
    ...
    get category(): FeedCategory[] | undefined {
        ...
        if (typeof this.feedOption.category === 'object') return [this.feedOption.category];

        return resolveCategory(this.frontmatter).map((item) => ({ name: item }));
    }
    ...
    get image(): string | undefined {
        ...
        if (banner) {
            if (isAbsoluteUrl(banner)) return resolveUrl(this.options.hostname, this.app.options.base, banner);
            if (isUrl(banner)) return banner;
        }

        if (cover) {
            if (isAbsoluteUrl(cover)) return resolveUrl(this.options.hostname, this.app.options.base, cover);
    ...
    get contributor(): FeedContributor[] {
        if (Array.isArray(this.feedOption.contributor)) return this.feedOption.contributor;

        if (typeof this.feedOption.contributor === 'object') return [this.feedOption.contributor];
    ...
    get copyright(): string | undefined {
        ...
        if (firstAuthor && firstAuthor.name) return `Copyright by ${firstAuthor.name}`;
    ...
    getFeedItem(): FeedItemOption | false {
        ...
        if (contributor) contributor.forEach((item) => this.feed.addContributor(item));
