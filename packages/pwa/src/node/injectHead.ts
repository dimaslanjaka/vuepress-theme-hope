...
export const injectLinkstoHead = (options: PWAOptions, base = '/', head: HeadConfig[] = []): HeadConfig[] => {
    ...
    let fallBackIcon = '';
    const setLink = (rel: string, href: string, more: Record<string, string> = {}): void => {
        if (!linkKeys.includes(rel)) head.push(['link', { rel, href, ...more }]);
    };

    const setMeta = (name: string, content: string, more: Record<string, string> = {}): void => {
        if (!metaKeys.includes(name)) head.push(['meta', { name, content, ...more }]);
    };
    ...
    if (options.manifest?.icons) {
        ...
        if (icons.length > 0) {
            ...
            options.manifest.icons.map((icon) => {
                if (icon.type) setLink('icon', icon.src, { type: icon.type, sizes: icon.sizes });
    ...
    if (typeof options.apple === 'object' && (options.apple.icon || fallBackIcon)) {
    ...
    if (typeof options.msTile === 'object' && (options.msTile.image || fallBackIcon)) {
