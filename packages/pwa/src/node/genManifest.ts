...
export const getManifest = async (options: PWAOptions, app: App): Promise<ManifestOption> => {
    ...
    const finalManifest: ManifestOption = {
        ...
        short_name: siteData.title || (themeConfig.title as string | undefined) || 'Site',
        description: siteData.description || 'A site built with vuepress-theme-hope',
...
export const genManifest = async (options: PWAOptions, app: App): Promise<void> => {
    console.log(blue('PWA:'), black.bgYellow('wait'), 'Generating manifest.webmanifest...');
    const { dir } = app;
    ...
    console.log(
        ...
        `Manifest generated and saved to ${cyan(relative(process.cwd(), manifestPath))}!`
    );
};
