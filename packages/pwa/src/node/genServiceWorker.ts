...
import type { ManifestEntry, ManifestTransform, ManifestTransformResult } from 'workbox-build';
...
export const genServiceWorker = async (options: PWAOptions, app: App): Promise<void> => {
    console.log(blue('PWA:'), black.bgYellow('wait'), 'Generating service worker...');
    const { dest } = app.dir;
    ...
    if (options.cacheHTML === false) globPatterns.push('./index.html', './404.html');
    ...
    await generateSW({
        ...
        console.log(
            ...
            `Generated service worker, which will precache ${cyan(`${count} files`)}, totaling ${cyan(
        ...
        if (size > 104857600)
            console.log(
                ...
                blue('Please consider disable `cacheHTML` and `cachePic`, or set `maxSize` and `maxPicSize` option.\n')
            );
    ...
    await writeFile(swDest, await readFile(resolve(__dirname, './skip-waiting.js'), 'utf8'), { flag: 'a' });
};
