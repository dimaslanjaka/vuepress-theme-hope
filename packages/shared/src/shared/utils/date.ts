...
const getLang = (lang = 'en'): string => {
    ...
    if (langcode === 'en-us' || langcode === 'en-uk' || langcode === 'en') return 'en';
...
export const timeTransformer = (date: Date, option: DateOptions = {}): string => {
    const { lang, timezone, type } = option;
    dayjs.locale(getLang(lang));

    const dateText = timezone ? dayjs(date).tz(timezone).format('LL') : dayjs(date).format('LL');

    const timeText = timezone ? dayjs(date).tz(timezone).format('HH:mm') : dayjs(date).format('HH:mm');

    return type === 'date' ? dateText : type === 'time' ? timeText : `${dateText} ${timeText}`;
};

export const getDate = (date: string | Date | undefined, options?: DateOptions): DateInfo | null => {
    if (date) {
