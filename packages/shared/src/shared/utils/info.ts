...
export const _getAuthor = (author: string[] | string | false | undefined, canDisable = false): string[] => {
    if (author) {
        if (Array.isArray(author)) return author;
        if (typeof author === 'string') return [author];

        console.error(
            `Expect 'author' to be \`string[] | string ${canDisable ? '' : '| false'} | undefined\`, but got`,
            author
        );
