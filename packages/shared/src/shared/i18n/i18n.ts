...
import type { App, AppOptions } from '@vuepress/core';
import type { HopeLang, HopeLangPath, HopeThemeLocaleConfigItem } from './types';
import type { BaseThemeConfig } from '../types';
...
export const getRootLang = (app: App): HopeLang => {
    ...
    if (siteLocales?.['/'] && checkLang(siteLocales['/']?.lang)) return siteLocales['/'].lang as HopeLang;
    ...
    if (themeLocales?.['/'] && checkLang(themeLocales['/']?.lang)) return themeLocales['/'].lang as HopeLang;
...
export const getRootLangPath = (app: App): HopeLangPath => lang2PathConfig[getRootLang(app)];
...
export const getDefaultLocale = (): HopeThemeLocaleConfigItem => localesConfig['en-US'];
