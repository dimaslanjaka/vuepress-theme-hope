...
const blockTex = (state: StateBlock, start: number, end: number, silent: boolean): boolean => {
    let firstLine;
    let lastLine;
    let next;
    let lastPos;
...
const katexInline = (tex: string, options: Katex.KatexOptions): string => {
    ...
    try {
        ...
        return `<span class='katex-error' title='${escapeHtml((error as Error).toString())}'>${escapeHtml(tex)}</span>`;
    }
};
...
const katexBlock = (tex: string, options: Katex.KatexOptions): string => {
    ...
    try {
        ...
        return `<p class='katex-block katex-error' title='${escapeHtml((error as Error).toString())}'>${escapeHtml(
            tex
        )}</p>`;
    }
};

export const katex = (md: MarkdownIt, options: katex.KatexOptions = { throwOnError: false }): void => {
    ...
    md.renderer.rules.inlineTex = (tokens, idx): string => katexInline(tokens[idx].content, katexOptions);
    md.renderer.rules.blockTex = (tokens, idx): string => `${katexBlock(tokens[idx].content, katexOptions)}\n`;
};
