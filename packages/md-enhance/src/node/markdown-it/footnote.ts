...
const renderFootnoteBlockOpen = (_tokens: FootNoteToken[], _idx: number, options: MarkdownIt.Options): string =>
    `${
...
const renderFootnoteOpen = (
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    `<li id="footnote${slf.rules.footnoteAnchorName!(tokens, idx, options, env, slf)}${getIDSuffix(
        tokens,
        idx
    )}" class="footnote-item">`;
...
const renderFootnoteAnchor = (
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    return ` <a href="#footnote-ref${self.rules.footnoteAnchorName!(tokens, idx, options, env, self)}${
        getIDSuffix(tokens, idx)
...
const footnoteDef = (state: FootNoteStateBlock, startLine: number, endLine: number, silent: boolean): boolean => {
    let pos;
    let token;
    let offset;
    let ch;
    ...
    offset = state.sCount[startLine] + pos - (state.bMarks[startLine] + state.tShift[startLine]);
...
const footnoteInline = (state: FootNoteStateInline, silent?: boolean): boolean => {
    let footnoteId;
    let token;
    let tokens: Token[];
...
const footnoteRef = (state: FootNoteStateInline, silent?: boolean): boolean => {
    ...
    if (!silent) {
        ...
        state.env.footnotes.list[footnoteId].count = (state.env.footnotes.list[footnoteId].count as number) + 1;
...
const footnoteTail = (state: FootNoteStateCore): boolean => {
    ...
    for (let i = 0, { length } = list; i < length; i++) {
        ...
        for (let j = 0; j < (Number(list[i].count) > 0 ? (list[i].count as number) : 1); j++) {
