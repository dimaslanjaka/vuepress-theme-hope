...
const startsWithTodoMarkdown = (token: Token): boolean =>
    token.content.indexOf('[ ] ') === 0 || token.content.indexOf('[x] ') === 0 || token.content.indexOf('[X] ') === 0;
...
const todoify = (token: Token, state: TaskListStateCore, options: Required<TaskListOptions>): void => {
...
export const tasklist = (md: MarkdownIt, { label = true, labelAfter = true }: TaskListOptions = {}): void => {
