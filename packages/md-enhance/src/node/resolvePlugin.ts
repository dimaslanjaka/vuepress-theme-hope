...
export const resolvePlugin = (markdownOption: MarkdownEnhanceOptions, app: App): PluginConfig[] => {
    const resolveConfig = (
    ...
    const config: PluginConfig[] = [
        ['@mr-hope/palette'],
        ['@vuepress/container', { type: 'info', defaultTitle: resolveConfig(i18n.info) }],
        ['@vuepress/container', { type: 'tip', defaultTitle: resolveConfig(i18n.tip) }],
        ['@vuepress/container', { type: 'warning', defaultTitle: resolveConfig(i18n.warning) }],
        ['@vuepress/container', { type: 'danger', defaultTitle: resolveConfig(i18n.danger) }],
        [
            '@vuepress/container',
            {
                type: 'details',
                before: (info: string): string =>
                    `<details class="custom-block details"><summary>${info || 'Details'}</summary>\n`,
    ...
    if (markdownOption.demo || markdownOption.enableAll)
        config.push(['@vuepress/container', { type: 'demo', render: codeDemoRender }]);
