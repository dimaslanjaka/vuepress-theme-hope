...
import type { CodeType, NormalCode, ReactCode, VueCode, VueScript } from './typings';
...
export const h = (tag: string, attrs: Record<string, string>, children?: HTMLElement[]): HTMLElement => {
...
const getHtmlTemplate = (html: string): string => `<div id="app">${handleHTML(html)}</div>`;
...
const getNormalScript = (js: string): (() => unknown) => {
    const script = window.Babel ? window.Babel.transform(js, { presets: ['es2015'] }).code : js;
...
export const getCode = (code: Record<string, string>): CodeType => {
    ...
    (['html', 'js', 'css'] as PreProcessorType[]).forEach((type) => {
        ...
        if (match.length) {
            ...
            result[type] = [code[language].replace(/^\n|\n$/g, ''), preProcessorConfig[type].map[language] || language];
        }
    });
...
export const getNormalCode = (code: CodeType, config: Partial<CodeDemoOptions>): NormalCode => ({
...
export const getVueCode = (code: CodeType, config: Partial<CodeDemoOptions>): VueCode => {
    ...
    const [js = '', jsLang = ''] = jsBlock ? [jsBlock[4].replace(/^\n|\n$/g, ''), jsBlock[3]] : [];
    const [css = '', cssLang = ''] = cssBlock ? [cssBlock[4].replace(/^\n|\n$/g, ''), cssBlock[3]] : [];
...
export const getReactCode = (code: CodeType, config: Partial<CodeDemoOptions>): ReactCode => {
    const scriptObj =
