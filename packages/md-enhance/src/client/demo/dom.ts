...
export const select = (node: Element | Document, selector: string): HTMLElement[] =>
...
const expandHandler = (
    ...
    codeWrapperNode.style.height = toBeExpand ? `${codeContainerNode.clientHeight + 13.8}px` : '0';
...
const getCodepenButton = ({ html, js, css, jsLib, cssLib }: Code, codeType?: CodeType): HTMLElement =>
    h(
        'form',
        {
...
export const initDom = ({ code, codeType, container, title }: DOMInitOption): void => {
    const { id } = container;
