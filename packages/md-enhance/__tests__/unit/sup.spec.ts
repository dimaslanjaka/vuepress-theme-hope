...
describe('superscript', () => {
    ...
    it('Should render when having other symbols', () => {
        expect(markdownIt.render(`^foo~bar^baz^bar~foo^`)).toEqual('<p><sup>foo~bar</sup>baz<sup>bar~foo</sup></p>\n');
    });
    ...
    it('Should work with other marker', () => {
        expect(markdownIt.render(`**^foo^ bar**`)).toEqual('<p><strong><sup>foo</sup> bar</strong></p>\n');
