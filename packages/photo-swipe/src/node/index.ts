...
const photoSwipePlugin: Plugin<PhotoSwipeOptions> = (options, app) => {
    const option =
        Object.keys(options).length > 0 ? options : (app.options.themeConfig.photoSwipe as PhotoSwipeOptions) || {};
    ...
    return {
        ...
        clientAppRootComponentFiles: path.resolve(__dirname, '../client/root-components/PhotoSwipe.js'),
