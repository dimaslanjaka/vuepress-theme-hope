    ...
    <div class="pagination-wrapper">
        <div v-if="enable" class="pagination-list">
            <div class="btn-group">
                ...
                <div v-if="displayRightEllipsis" role="navigation" @click="navigate(totalPages)">
                    {{ totalPages }}
                </div>
                <div v-if="currentPage < totalPages" class="next" role="navigation" @click="navigate(currentPage + 1)">
                    {{ i18n.next }}
                </div>
            </div>
            <div class="navigate-wrapper">
                <label for="navigation-text">{{ i18n.navigate }}:&nbsp;</label>
                <input id="navigation-text" v-model="input" type="text" @keypress.enter="jumpPage(input)" />
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const i18n = computed(() => componentI18n[routeLocale.value || '/'].pagination);
