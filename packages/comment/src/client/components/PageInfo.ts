import { useIconPrefix, useThemePluginConfig } from '@mr-hope/vuepress-shared/client';
...
import type { CommentOptions, CommentPluginFrontmatter, PageInfoType } from '../../shared';
            ...
            h('div', { class: 'page-title' }, [
                h('h1', [
                    frontmatter.value.icon
                        ? h('i', {
                              class: ['iconfont', `${iconPrefix.value}${frontmatter.value.icon}`],
                          })
                        : null,
                    page.value.title,
                ]),
                config.value
                    ? h('div', { class: 'page-info' }, [
                          isOriginal.value ? h('span', { class: 'origin' }, originText.value) : null,
