...
import { usePageFrontmatter, usePageLang, useRouteLocale } from '@vuepress/client';
import { computed, defineComponent, h, nextTick, onBeforeUnmount, onMounted, watch } from 'vue';
...
export default defineComponent({
    ...
    setup() {
        ...
        const enablePageViews = computed(() => resolveEnablePageViews(frontmatter.value));
        ...
        const initWaline = (path: string): void => {
            void import(/* webpackChunkName: "waline" */ '@waline/client').then(({ default: Waline }) => {
                new Waline({
                    ...
                    placeholder: walineOption.placeholder || walineI18n[routeLocale.value],
        ...
        onMounted(() => {
            if (enableWaline) timeout = setTimeout(() => initWaline(route.path), 1000);
        });
