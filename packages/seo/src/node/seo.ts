...
export const generateSeo = (options: SeoOptions, base: string, { page, app, permalink }: PageSeoInfo): SeoContent => {
    const {
        frontmatter: { author: pageAuthor, date, banner, cover, tag, tags = tag as string[] },
        lastUpdatedTime,
    } = page;
    const { siteData } = app;
    ...
    const type = ['article', 'category', 'tag', 'timeline'].some(
        (folder) => page.filePathRelative && page.filePathRelative.startsWith(`/${folder}`)
    )
        ? 'website'
        : 'article';
    ...
    const author =
        pageAuthor === false
            ? ''
            : (pageAuthor as string) || options.author || (app.options.themeConfig.author as string | undefined) || '';
    const modifiedTime = typeof lastUpdatedTime === 'number' ? new Date(lastUpdatedTime).toISOString() : '';
    const articleTags: string[] = Array.isArray(tags) ? tags : typeof tag === 'string' ? [tag] : [];
    ...
    return {
        ...
        'og:type': type,
        'og:image': cover ? resolveUrl(base, cover) : banner ? resolveUrl(base, banner) : '',
...
export const generateRobotsTxt = async (dir: AppDir): Promise<void> => {
    ...
    let content = existsSync(publicPath) ? await readFile(publicPath, { encoding: 'utf8' }) : '';

    if (content && !content.includes('User-agent'))
        console.log(blue('SEO:'), black.bgRed('error'), 'robots.txt seems invalid!');
    else content += '\nUser-agent:*\nDisallow:\n';
    ...
    console.log(blue('SEO:'), black.bgGreen('Success'), `${cyan('robots.txt')} generated`);
};
