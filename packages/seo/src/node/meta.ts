...
const addMeta = (
    meta: HeadConfig[],
    {
        name,
        content,
        attribute = ['article:', 'og:'].some((type) => name.startsWith(type)) ? 'property' : 'name',
    }: MetaOptions
): void => {
    if (content) meta.push(['meta', { [attribute]: name, content }]);
};

export const appendMeta = (head: HeadConfig[], content: SeoContent, options: SeoOptions): void => {
    ...
    if (options.twitterID) addMeta(head, { name: 'twitter:creator', content: options.twitterID });
};
