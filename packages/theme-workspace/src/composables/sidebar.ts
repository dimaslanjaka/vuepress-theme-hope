...
import type { ThemeHopeOptions, SidebarConfigArray, SidebarConfigObject, ResolvedSidebarItem } from '../types';
 ...
 */
export const headerToSidebarItem = (header: PageHeader): ResolvedSidebarItem => ({
    text: header.title,
    link: `#${header.slug}`,
 ...
 */
export const resolveArraySidebarItems = (sidebarConfig: SidebarConfigArray): ResolvedSidebarItem[] => {
 ...
 */
export const resolveMultiSidebarItems = (sidebarConfig: SidebarConfigObject): ResolvedSidebarItem[] => {
    const route = useRoute();
...
export const resolveSidebarItems = (): SidebarItemsRef => {
    ...
    const sidebarConfig = computed(
        () => (frontmatter.value.sidebar as ThemeHopeOptions['sidebar']) || themeLocale.value.sidebar
    );
