...
export const clickOutsideDirectice: Directive<PopupHtmlElement, ClickOutHandler> = {
    beforeMount: (element, binding, vNode) => {
        ...
        const handler = (event: Event): void => {
            ...
            const elements: Node[] =
                // eslint-disable-next-line
                (event as any).path || (event.composedPath ? (event.composedPath() as Node[]) : []);
    ...
    beforeUpdate: (element, binding) => {
        if (validate(binding) && element.$clickOutside) element.$clickOutside.callback = binding.value;
    },
